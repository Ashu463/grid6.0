...
describe('CartService', () => {
  ...
  describe('getCart', () => {
    ...
    //   const data: getCartDTO = { userId: "testUser", id: 'testId' }
    //   await expect(service.getCart(data)).rejects.toThrow(BadRequestException);
    // });

    it('should throw NotFoundException if cart not found', async () => {
      const data: getCartDTO = { userId: "testUser", id: 'testId' }
      jest.spyOn(prismaService.cart, 'findUnique').mockResolvedValue(null);
      await expect(service.getCart(data)).rejects.toThrow(NotFoundException);
    });
  ...
  describe('addItemToCart', () => {
    ...
    //   const addItemToCartDto: AddItemToCartDto = { productId: 'product123', quantity: 1, userId: 'testuser', id: 'testId' };

    //   await expect(service.addItemToCart(addItemToCartDto)).rejects.toThrow(NotFoundException);
    // });
    ...
    //   const addItemToCartDto: AddItemToCartDto = { productId: 'product123', quantity: 1, userId: 'testuser', id: 'testId' };
    //   await expect(service.addItemToCart(addItemToCartDto)).resolves.toEqual(addedItem);
    // });
  });
  ...
  describe('removeItemFromCart', () => {
    ...
    //   expect(await service.removeItemFromCart('item123')).toEqual({ success : false, message: 'Item removed from cart' });
    // });
  });
  ...
  describe('clearCart', () => {
    it('should throw NotFoundException if cart is not found', async () => {
      ...
      await expect(service.clearCart('user123')).rejects.toThrow(BadRequestException);
    });
    ...
    //   await expect(service.clearCart('user123')).resolves.toEqual({ success : true, message: 'Cart cleared' });
    // });
  });
})
